            ...
            @forelse($meta->recommendations as $recommendation)
                <div class="item mini backdrop mini_card">
                    <div class="image_content">
                        <a href="{{ route('torrents.similar', ['category_id' => $torrent->category_id, 'tmdb' => $recommendation->recommendation_movie_id ?? $recommendation->recommendation_tv_id ?? $recommendation->recommendation_cartoon_id ?? $recommendation->recommendation_cartoontv_id]) }}">
                            <div>
            ...
            @empty
                <div class="text-center">
                    <h4 class="text-bold text-danger">
                        <i class="{{ config('other.font-awesome') }} fa-frown"></i> Najdeno ni bilo nobenih priporočil!
                    </h4>
                </div>
            @endforelse
            @endif
