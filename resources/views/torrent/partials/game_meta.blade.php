...
@endif
    <div class="meta-info">
                    ...
                    <a href="{{ $meta->involved_companies[0]['company']['url'] }}" target="_blank">
                        @if(array_key_exists("logo", $meta->involved_companies[0]['company']))
                            <img class="img-responsive"
                                 src="{{ $meta->involved_companies[0]['company']['logo']['image_id'] ? 'https://images.igdb.com/igdb/image/upload/t_logo_med/'.$meta->involved_companies[0]['company']['logo']['image_id'].'.png' : '/img/SLOshare/games_no_image_138x175.png' }}">
                        @endif
                    </a>
                </div>
            @endif
        ...
        <div class="movie-backdrop"
             style="background-image: url('{{ (isset($meta) && $meta->artworks) ? 'https://images.igdb.com/igdb/image/upload/t_screenshot_big/'.$meta->artworks[0]['image_id'].'.jpg' : '/img/SLOshare/games_no_image_banner.jpg' }}');"></div>

        <div class="movie-top">
            <h1 class="movie-heading" style="margin-bottom: 0;">
                <span class="text-bright text-bold" style="font-size: 28px;">{{ $meta->name ?? 'Meta ni bila najdena' }}</span>
        ...
        <div class="movie-bottom">
            ...
            <span class="badge-user text-bold">
                <i class="fab fa-xbox"></i> Platforme:
            </span>
            <div class="cast-list">
                @if (isset($platforms))
                    @foreach ($platforms as $platform)
                        <div class="cast-item" style="max-width: 80px;">
                            <img class="img-responsive"
                                 src="{{ $platform->image_id ? 'https://images.igdb.com/igdb/image/upload/t_logo_med/'.$platform->image_id.'.png' : '/img/SLOshare/games_no_image_138x175.png' }}">
                        </div>
                    @endforeach
                @endif
            </div>
