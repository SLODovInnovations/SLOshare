...
@section('title')
    <title>{{ __('torrent.download-check') }}</title>
@endsection
        ...
        <div class="well">
            <div class="text-center">
                <strong>{{ __('common.ratio') }} {{ strtolower(__('torrent.greater-than')) }} {{ config('other.ratio') }}
                    : </strong>
                @if ($user->getRatio() < config('other.ratio'))<span class="text-red"><i
                            class="{{ config('other.font-awesome') }} fa-times"></i>
                        {{ strtoupper(__('torrent.failed')) }}</span>
                @else<span class="text-green"><i class="{{ config('other.font-awesome') }} fa-check"></i>
                            {{ strtoupper(__('torrent.passed')) }}</span>
                @endif
                <strong>{{ __('torrent.download-rights-active') }}: </strong>
                @if ($user->can_download == 0 && $torrent->user_id != $user->id)<span class="text-red"><i
                            class="{{ config('other.font-awesome') }} fa-times"></i>
                            {{ strtoupper(__('torrent.failed')) }}</span>
                @else<span class="text-green"><i class="{{ config('other.font-awesome') }} fa-check"></i>
                            {{ strtoupper(__('torrent.passed')) }}</span>
                ...
                @if ($torrent->isRejected())<span class="text-red"><i
                            class="{{ config('other.font-awesome') }} fa-times"></i>
                            {{ strtoupper(__('torrent.rejected')) }}</span>
                @elseif ($torrent->isPending())<span class="text-orange"><i
                            class="{{ config('other.font-awesome') }} fa-times"></i>
