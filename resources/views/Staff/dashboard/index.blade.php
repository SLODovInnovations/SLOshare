...
@section('title')
    <title>{{ __('staff.staff-dashboard') }}</title>
@endsection
                ...
                @if (auth()->user()->group->is_owner)
                    ...
                    <p class="form__group form__group--horizontal">
                        <a class="form__button form__button--text" href="{{ route('staff.commands.index') }}">
                            <i class="fab fa-laravel"></i> Ukazi
                        </a>
                    </p>
                @endif
            </div>
                    ...
                    <form method="POST" action="{{ route('staff.flush.chat') }}" x-data>
                        ...
                        <button
                            x-on:click.prevent="Swal.fire({
                                ...
                                text: 'Ali ste prepričani, da želite izbrisati vsa sporočila klepetalnice v vseh klepetalnicah (vključno z zasebnimi sporočili klepetalnice)?',
                                icon: 'warning',
                ...
                <p class="form__group form__group--horizontal">
                    <form method="POST" action="{{ route('staff.flush.peers') }}" x-data>
                        ...
                        <button
                            x-on:click.prevent="Swal.fire({
                                title: 'Ali si prepričan?',
                                text: 'Ali ste prepričani, da želite izbrisati vse vrstnike duhove?',
                                icon: 'warning',
                                showConfirmButton: true,
                                showCancelButton: true,
                            }).then((result) => {
                ...
                <p class="form__group form__group--horizontal">
                    <form method="GET" action="{{ route('staff.mass-actions.validate') }}" x-data>
                        ...
                        <button
                            x-on:click.prevent="Swal.fire({
                                title: 'Ali si prepričan?',
                                text: 'Ali ste prepričani, da želite samodejno potrditi vse uporabnike, tudi če njihov E-Mail ni potrjen?',
                                icon: 'warning',
                                showConfirmButton: true,
                                showCancelButton: true,
                            }).then((result) => {
...
@section('sidebar')
    <section class="panelV2">
        ...
        <dl class="key-value">
            ...
            @if (request()->secure())
                ...
                <dd>Varno</dd>
                <dt>Izdal</dt>
                <dd>{{ (!is_string($certificate)) ? $certificate->getIssuer() : "Ni podatkov o potrdilu" }}</dd>
                <dt>Expires</dt>
                <dd>{{ (!is_string($certificate)) ? $certificate->expirationDate()->diffForHumans() : "Ni podatkov o potrdilu" }}</dd>
            @else
                <dt>Povezava</dt>
                <dd>
                    <strong>Ni varno</strong>
                </dd>
                <dt>Izdal</dt>
                <dd>N/A</dd>
                <dt>Poteče</dt>
    ...
    <section class="panelV2">
        <h2 class="panel__heading">Informacije o strežniku</h2>
        <dl class="key-value">
            <dt>OS</dt>
            <dd>{{ $basic['os'] }}</dd>
            <dt>PHP</dt>
            ...
            <dt>Laravel</dt>
            <dd>{{ $basic['laravel'] }}</dd>
            <dt>{{ config('sloshare.codebase') }}</dt>
            <dd>{{ config('sloshare.version') }}</dd>
        </dl>
    </section>
    <div class="dashboard__stats">
        <section class="panelV2 panel--grid-item">
            <h2 class="panel__heading">Torrenti</h2>
            <dl class="key-value">
                <dt>Skupaj</dt>
        ...
        <section class="panelV2 panel--grid-item">
            <h2 class="panel__heading">Vrstniki</h2>
            <dl class="key-value">
                <dt>Skupaj</dt>
                <dd>{{ $peers->total }}</dd>
                <dt>Seeds</dt>
        ...
        <section class="panelV2 panel--grid-item">
            <h2 class="panel__heading">Uporabniki</h2>
            <dl class="key-value">
                <dt>Seeds</dt>
                <dd>{{ $users->total }}</dd>
                <dt>Potrjevanje</dt>
    ...
    <section class="panelV2">
        <h2 class="panel__heading">Dovoljenja imenika</h2>
