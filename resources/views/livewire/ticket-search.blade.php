        ...
        <table class="table vertical-align table-hover">
            ...
            @foreach ($tickets as $ticket)
                <tr>
                    ...
                    <td>
						<span class="badge-user">
							@if($ticket->priority->name === 'Nizko')
                                <i class="fas fa-circle text-yellow"></i>
                            @elseif ($ticket->priority->name === 'Srednje')
                                <i class="fas fa-circle text-orange"></i>
                            @elseif ($ticket->priority->name === 'Visoko')
                    ...
                    <td>
						<span class="badge-user">
							@if($ticket->closed_at)
                                <i class="fas fa-circle text-danger"></i> Zaprto
                            @else
                                <i class="fas fa-circle text-success"></i> Odprto
                            @endif
						</span>
                    </td>
                    <td>
                    ...
                    <td>
						<span class="badge-user">
							{{ date('d.m.Y h:i:s', $ticket->created_at->getTimestamp()) }}
						</span>
                    </td>
                    <td>
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li role="presentation">
                                    <a role="menuitem" tabindex="-1" target="_blank"
                                       href="{{ route('tickets.show', ['id' => $ticket->id]) }}">Pogled</a>
                                </li>
                                <li role="presentation">
                                    <form method="POST" action="{{ route('tickets.close', ['id' => $ticket->id]) }}">
										...
										font-weight: 400; padding: 7px 15px;">Zaprto
                                        </button>
                                    </form>
                                </li>
                            </ul>
