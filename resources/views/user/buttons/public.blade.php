<div class="button-holder">
    <div class="button-left-increased">
        ...
        <!--@if (auth()->user()->isAllowed($user,'achievement','show_achievement'))
            <a href="{{ route('achievements.show', ['username' => $user->username]) }}" class="btn btn-primary">
                {{ __('user.achievements') }}
            </a>
        @endif-->
        @if (auth()->user()->isAllowed($user,'follower','show_follower'))
            <a href="{{ route('user_followers', ['username' => $user->username]) }}" class="btn btn-primary">
                {{ __('user.followers') }}
            </a>
        @endif
    ...
    <div class="button-right-decreased">
        @if (auth()->user()->id != $user->id)
            ...
            @endif
            <button class="btn btn-danger" data-toggle="modal" data-target="#modal_user_report"><i
