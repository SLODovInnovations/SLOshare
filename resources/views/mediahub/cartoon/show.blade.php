...
@section('content')
    <div class="container">
        <div class="block single">
            @include('torrent.partials.cartoon_meta', ['torrent' => $cartoon->torrents->first(), 'meta' => $cartoon])

            <div class="table-responsive mt-20">
                <table class="table table-condensed table-bordered table-striped">
                    @foreach(App\Models\Type::all()->sortBy('position') as $type)
                        @if($cartoon->torrents->where('type_id', '=', $type->id)->count() > 0)
                            <thead>
                            <tr>
                        ...
                        @foreach(App\Models\Resolution::all()->sortBy('position') as $resolution)
                            @if($cartoon->torrents->where('type_id', '=', $type->id)->where('resolution_id', '=', $resolution->id)->count() > 0)
                                <thead>
                                <tr>
                                    <th colspan="6">
                            ...
                            @endif
                            <tbody>
                            @foreach($cartoon->torrents->where('type_id', '=', $type->id)->where('resolution_id', '=', $resolution->id)->sortByDesc('created_at') as $torrent)
                                @if($torrent->sticky == 1)
                                    <tr class="success">
                                @else
                                    <tr>
