            ...
            <div class="row">
                ...
                @foreach($torrents as $t)
                    ...
                    @if ($t->torrent->category->cartoons_meta)
                        @if ($t->torrent->tmdb || $t->torrent->tmdb != 0)
                            @php $meta = App\Models\Cartoons::with('genres', 'cast', 'companies', 'collection')->where('id', '=', $t->torrent->tmdb)->first() @endphp
                        @endif
                    @endif
                    <div class="col-md-6">
                                ...
                                <div class="body_description">
                                    <h3 class="description_title">
                                        <a href="{{ route('torrent', ['id' => $t->torrent->id]) }}">{{ $t->torrent->name }}
                                            @if($t->torrent->category->movie_meta || $t->torrent->category->cartoons_meta || ($t->torrent->category->tv_meta && isset($t->torrent->meta) && $meta->releaseYear))
                                    ...
                                    <p class="description_plot">
                                        @if($t->torrent->category->movie_meta || $t->torrent->category->cartoons_meta || ($t->torrent->category->tv_meta && $meta && $meta->plot))
                                            {{ $meta->overview ?? '' }}
                                        @endif
                                    </p>
                                </div>
