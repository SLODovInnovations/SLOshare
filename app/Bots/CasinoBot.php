...
class CasinoBot
{
    ...
    public function getDonations(string $duration = 'default'): string
    {
        ...
        return "Najnovejše donacije zame so naslednje:\n\n".\trim($donationDump);
    }
    ...
    public function process($type, User $user, string $message = '', int $targeted = 0): \Illuminate\Contracts\Routing\ResponseFactory|\Illuminate\Http\Response|bool
    {
        ...
        if ($message === '') {
            ...
            $log = 'Vsi '.$this->bot->name.' ukazi morajo biti zasebno sporočilo ali začeti z /'.$this->bot->command.' oz !'.$this->bot->command.'. Rabim pomoč? Vrsta /'.$this->bot->command.' pomagaj in pomagali ti bodo.';
        }
