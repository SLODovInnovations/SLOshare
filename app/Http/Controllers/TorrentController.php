...
class TorrentController extends Controller
{
    ...
    public function similar(int $categoryId, int $tmdbId): \Illuminate\Contracts\View\Factory|\Illuminate\View\View
    {
        ...
        if (! $torrent || $torrent->count() === 0) {
            \abort(404, 'Podobnih torrentov ni bilo mogoče najti');
        }
    ...
    private static function anonymizeMediainfo($mediainfo): array|string|null
    {
        ...
        $completeNameI = \strpos($mediainfo, 'Popolno ime');
    ...
    public function upload(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        return \to_route('download_check', ['id' => $torrent->id])
            ->withSuccess('Vaša torrent datoteka je pripravljena za prenos in seme!');
    }
    ...
    public function download(Request $request, int $id, $rsskey = null): \Illuminate\Http\RedirectResponse|\Symfony\Component\HttpFoundation\BinaryFileResponse
    {
        ...
        if ($user->getRatio() < \config('other.ratio') && ! ($torrent->user_id === $user->id || $hasHistory)) {
            return \to_route('torrent', ['id' => $torrent->id])
                ->withErrors('Vaše razmerje je nizko za prenos!');
        }

        // User's download rights are revoked
        if ($user->can_download == 0 && ! ($torrent->user_id === $user->id || $hasHistory)) {
            return \to_route('torrent', ['id' => $torrent->id])
                ->withErrors('Vaše pravice za prenos so bile preklicane!');
        }
        ...
        if (! \file_exists(\getcwd().'/files/torrents/'.$torrent->file_name)) {
            return \to_route('torrent', ['id' => $torrent->id])
                ->withErrors('Torrent datoteka ni najdena! Prosimo, poročajte o Torrentu!');
        }
    ...
    public function reseedTorrent(Request $request, int $id): \Illuminate\Http\RedirectResponse
    {
        ...
        if ($torrent->seeders <= 2) {
            ...
            $this->chatRepository->systemMessage(
                \sprintf('Dame in Gospodje, pravkar je bila poslana zahteva za ponovno sejanje [url=%s]%s[/url] lahko pomagaš :question:', $torrentUrl, $torrent->name)
            );

            return \to_route('torrent', ['id' => $torrent->id])
                ->withSuccess('Obvestilo je bilo poslano vsem uporabnikom, ki so prenesli ta torrent skupaj s prvotnim prenosnikom!');
        }

        return \to_route('torrent', ['id' => $torrent->id])
            ->withErrors('Ta torrent ne izpolnjuje pravil za zahtevo po ponovnem zagonu.');
    }
}
