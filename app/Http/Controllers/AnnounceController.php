...
class AnnounceController extends Controller
{
    ...
    protected function checkPasskey($passkey): void
    {
        ...
        // MD5 char limit
        \throw_if(\strspn(\strtolower($passkey), 'abcdef0123456789') !== 32, new TrackerException(131, [':attribute' => 'passkey', ':reason' => 'Oblika gesla ni pravilna']));
    }
    ...
    protected function checkTorrent($infoHash): object
    {
        ...
        \throw_if($torrent->status === self::PENDING, new TrackerException(151, [':status' => 'V ČAKANJU Moderacija']));

        // If Torrent Is Rejected Return Error to Client
        \throw_if($torrent->status === self::REJECTED, new TrackerException(151, [':status' => 'ZAVRNJENO Moderiranje']));

        // If Torrent Is Postponed Return Error to Client
        \throw_if($torrent->status === self::POSTPONED, new TrackerException(151, [':status' => 'PRESTAVLJENO Moderiranje']));
