    ...
    public function sendGift(Request $request): \Illuminate\Http\RedirectResponse
    {
        ...
        if ($v->passes()) {
            ...
            $this->chatRepository->systemMessage(
                \sprintf('[url=%s]%s[/url] je obdaril %s BON za [url=%s]%s[/url]', $profileUrl, $user->username, $value, $recipientUrl, $recipient->username)
            );
        ...
        if ($v->passes()) {
            $recipient = User::where('username', 'VŠEČ', $request->input('to_username'))->first();
