...
{
    protected $signature = 'vendor:cleanup {--check : Deluje v suhem načinu brez brisanja datotek.}';
     ...
     * @var string
     */
    protected $description = 'Počisti neuporabne datoteke iz mape prodajalca.';
    ...
    public function handle(): void
    {
        ...
        foreach ($directories as $directory) {
            foreach ($patterns as $pattern) {
                ...
                foreach ($files as $file) {
                    if (\is_dir($file)) {
                        $this->out('BRISANJE MAP: '.$file);
                        if (! $isDry) {
                            $this->delTree($file);
                        }
                    } else {
                        $this->out('BRISANJE DATOTEK: '.$file);
                        if (! $isDry) {
                            @\unlink($file);
                        }
                    }
        ...
        $this->out('Opravljeno Čiščenje prodajalca!');
    }
