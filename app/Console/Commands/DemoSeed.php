...
class DemoSeed extends Command
{
     ...
     * @var string
     */
    protected $description = 'Semena Ponarejeni podatki za demonstracijo ali testiranje';
    ...
    {
        $this->alert('Demo Seeder v2.0 (Avtor: SLOshare)');
        $this->warn('*** Njegov postopek lahko traja nekaj minut ***');
        $this->warn('Pritisnite CTRL + C za prekinitev');
        ...
        foreach ($this->movie_ids() as $key => $id) {
            // Users
            $this->info('Ustvarjanje uporabniškega računa');

            $uid = User::factory()->create([
                'chatroom_id'    => 1,
                'group_id'       => random_int(1, 13),
                'chat_status_id' => 1,
                'image'          => null,
                'custom_css'     => null,
                'locale'         => 'en',
            ...
            if ([false, true][random_int(0, 1)]) {
                ...
                // Torrents
                $this->info('Ustvarjanje filmskih torrentov za račun ID #'.$uid);
...
01:44:15.249                             : en:End Credits
        ...
        foreach ($this->tv_ids() as $key => $id) {
            ...
            if ([false, true][random_int(0, 1)]) {
                ...
                // Torrents
                $this->info('Ustvarjanje TV torrentov za ID računa #'.$uid);
...
01:44:15.249                             : en:End Credits
        ...
        if ($abort) {
            $this->error('Prekinjeno ...');
            $this->alert('Demo podatki so bili le DELNO zasejani! To je verjetno posledica časovne omejitve zahteve API.');
            $this->alert('Prepričajte se, da je ključ TMDB api nastavljen in zagnan "php artisan config:clear"');
        } else {
            $this->alert('Demo podatki so bili uspešno zasejani!');
        }
    }
